...
io.on('connection', (socket) => {
  ...
  if(PLAYERS.length === 2 ){
   ...
   PLAYERS[0].socket.emit('GAME_CREATED', { number: NUMBER, isStarted: true, canPlay: true })
   PLAYERS[1].socket.emit('GAME_CREATED', { number: NUMBER, isStarted: true, canPlay: false })
  }
  // ama bu sekilde yazinca kimin kazandigini tespit etmiyoruz aslinda?
  // son hamleyi yapan 
